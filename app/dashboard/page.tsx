...
export default function UsuariosPage() {
  ...
  const handleDeleteUser = async (id: string) => {
    if(!confirm('¿Eliminar usuario de forma permanente?')) return
    try {
      const response = await fetch('/api/users', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 bg-[#050505] p-8 rounded-[2rem] border border-white/5">
        ...
        <div className="flex gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input type="text" placeholder="BUSCAR..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl pl-12 pr-4 py-3.5 text-white text-[11px] font-bold uppercase tracking-widest focus:border-purple-500 outline-none transition-colors" />
          </div>
          <button onClick={()=>setShowInviteModal(true)} className="bg-purple-500 hover:bg-purple-400 text-white px-6 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-lg shadow-purple-500/20"><Plus size={16}/> AGREGAR</button>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                ...
                <th className="py-5 px-8 text-[10px] font-black text-zinc-500 tracking-[0.3em] uppercase">Nivel</th>
                <th className="py-5 px-8 text-[10px] font-black text-zinc-500 tracking-[0.3em] uppercase">Ingreso</th>
                <th className="py-5 px-8 text-[10px] font-black text-zinc-500 tracking-[0.3em] uppercase text-right">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={4} className="py-20 text-center"><Loader2 className="w-8 h-8 animate-spin text-purple-500 mx-auto mb-4" /><span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Sincronizando...</span></td></tr>
              ) : filteredUsers.length === 0 ? (
                <tr><td colSpan={4} className="py-20 text-center"><Users className="text-zinc-500 mx-auto mb-4" size={24} /><span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Sin registros</span></td></tr>
              ) : (
                filteredUsers.map((user, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors last:border-0 group">
                    <td className="py-5 px-8">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-purple-500/10 flex items-center justify-center text-sm font-black text-purple-500 uppercase border border-purple-500/20">{user.full_name?.charAt(0)}</div>
                        <div>
                          <p className="text-sm font-black text-white uppercase tracking-tighter">{user.full_name}</p>
                          <p className="text-[10px] text-zinc-500 font-bold flex items-center gap-1.5"><Mail size={10} /> {user.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-5 px-8">
                    ...
                    <td className="py-5 px-8"><span className="text-[10px] font-bold text-zinc-500 italic uppercase">{new Date(user.created_at).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' })}</span></td>
                    <td className="py-5 px-8 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={()=>handleResetPassword(user)} className="p-2 text-zinc-600 hover:text-amber-500 hover:bg-amber-500/10 rounded-xl transition-all"><Key size={16}/></button>
                        <button onClick={()=>handleDeleteUser(user.id)} className="p-2 text-zinc-600 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all"><Trash2 size={16}/></button>
                      </div>
                    </td>
                  </tr>
                ))

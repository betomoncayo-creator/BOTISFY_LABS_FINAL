...
export default function AcademiaPage() {
  ...
  // Rol derivado del contexto — sin llamada extra a Supabase
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      try {
        ...
        const coursesData = await db.getCoursesByRole(session.user.id, profile.role || 'estudiante')
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, courseId: string) => {
    ...
    try {
      ...
      const publicUrl = await db.uploadFile('course_images', fileName, file)
      await db.updateCourse(courseId, { image_url: publicUrl })
      setCourses(prev => prev.map(c => c.id === courseId ? { ...c, image_url: publicUrl } : c))
    } catch (error) {
  ...
  const removeImage = async (courseId: string) => {
    ...
    try {
      await db.updateCourse(courseId, { image_url: null })
  ...
  const handleEditCourse = async () => {
    if (!newCourseName.trim()) { alert('El nombre del curso es requerido'); return }
    if (!editingCourseId) return

    setCreatingCourse(true)
    try {
      const finalMinutes = durationUnit === 'hours' ? newCourseDuration * 60 : newCourseDuration
      await db.updateCourse(editingCourseId, {
        title: newCourseName,
      ...
      setNewCourseName(''); setNewCourseDesc(''); setNewCourseDuration(0)
      setDurationUnit('minutes'); setEditingCourseId(null); setShowEditCourseModal(false)
  ...
  const handleCreateCourse = async () => {
    ...
    try {
      ...
      setNewCourseName(''); setNewCourseDesc(''); setNewCourseDuration(0)
      setDurationUnit('minutes'); setShowAddCourseModal(false)
          ...
          <div className="bg-[#050505] border border-white/10 rounded-[2rem] p-8 max-w-md w-full space-y-6 animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-2">
              ...
              <textarea value={newCourseDesc} onChange={(e) => setNewCourseDesc(e.target.value)}
                placeholder="Describe el contenido..." rows={3}
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input type="number" value={newCourseDuration || ''} onChange={(e) => setNewCourseDuration(Number(e.target.value))}
                  min={0} placeholder="0"
                  className="w-full px-4 py-3 bg-zinc-900/50 border border-white/10 rounded-xl text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#00E5FF] transition-all text-sm" />
              ...
              {newCourseDuration > 0 && (
                <p className="text-zinc-500 text-[8px] font-bold uppercase tracking-widest">
                  = {durationUnit === 'hours' ? `${newCourseDuration * 60} minutos totales` : `${(newCourseDuration / 60).toFixed(1)} horas totales`}
                </p>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#050505] border border-white/10 rounded-[2rem] p-8 max-w-md w-full space-y-6 animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={handleEditCourse} disabled={creatingCourse || !newCourseName.trim()}
                className="flex-1 py-3 bg-[#00E5FF] text-black rounded-xl font-black text-[9px] uppercase hover:bg-[#00D4EE] transition-all disabled:opacity-50 disabled:cursor-not-allowed">

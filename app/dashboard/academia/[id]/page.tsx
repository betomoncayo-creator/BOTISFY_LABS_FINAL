...
export default function CourseEditorPage() {
  ...
  // 🛠️ FUNCIONES DE GESTIÓN DE NODOS
  ...
  // 🧠 LÓGICA DE SCORING CON PENALIZACIÓN
  const runSimulation = () => {
    ...
    objectiveQs.forEach((q: any) => {
      ...
      if (scorePregunta < 0) scorePregunta = 0 
    ...
    <div className="w-full space-y-8 pb-20 text-white animate-in fade-in">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-white/5 pb-8">
        <div className="flex items-center gap-6">
          <button onClick={() => router.back()} className="p-3 bg-white/5 rounded-xl border border-white/5 transition-all"><ChevronLeft size={20} /></button>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handlePublish} disabled={isSaving} className="bg-[#00E5FF] text-black px-8 py-3 rounded-xl font-black text-[10px] uppercase flex items-center gap-2">
              ...
              <div className="bg-[#050505] border border-white/5 p-10 rounded-[3rem] space-y-8 animate-in slide-in-from-right-4">
                ...
                {selectedModule.type === 'quiz' && (
                  <div className="space-y-8">
                    {!quizPreviewMode ? (
                      ...
                      <div className="space-y-8 animate-in slide-in-from-bottom-4 bg-white/[0.01] p-10 rounded-[2rem] border border-white/5 shadow-2xl">
                        {simulationResult && (
                        ...
                        {!simulationResult && <button onClick={runSimulation} className="w-full bg-[#00E5FF] text-black py-6 rounded-3xl font-black text-xs uppercase tracking-[0.3em] shadow-[0_0_40px_rgba(0,229,255,0.2)] hover:scale-[1.01] transition-all">Procesar Evaluación Neural</button>}
                      </div>
                    )}
                  </div>
                )}
                ...
                {selectedModule.type === 'pdf' && (
                  <div className="space-y-6">
                    {selectedModule.content && <div className="bg-white/5 rounded-2xl border border-white/10 h-[500px] overflow-hidden shadow-2xl"><iframe src={`${selectedModule.content}#toolbar=0`} className="w-full h-full border-none" /></div>}
                    <div className="relative group">
                      ...
                      <div className="w-full bg-white/5 border-2 border-dashed border-white/10 p-12 rounded-[2rem] text-center group-hover:border-[#00E5FF]/20 transition-all"><UploadCloud className="mx-auto mb-4 text-zinc-700 group-hover:text-[#00E5FF]" size={40} /><p className="text-zinc-500 text-[9px] font-black uppercase">Subir PDF Instructivo</p></div>
                    </div>
                  </div>
                )}
                ...
                {selectedModule.type === 'video' && (
                  <div className="space-y-6">
                    <div className="flex gap-2 p-1 bg-white/5 rounded-xl w-fit">
                      <button onClick={() => updateModule('videoSource', 'url')} className={`px-4 py-2 rounded-lg text-[8px] font-black uppercase transition-all ${selectedModule.videoSource === 'url' ? 'bg-[#00E5FF] text-black' : 'text-zinc-500 hover:text-white'}`}>Link</button>
                      <button onClick={() => updateModule('videoSource', 'file')} className={`px-4 py-2 rounded-lg text-[8px] font-black uppercase transition-all ${selectedModule.videoSource === 'file' ? 'bg-[#00E5FF] text-black' : 'text-zinc-500 hover:text-white'}`}>Archivo</button>
                    </div>
                    {selectedModule.content && (
                      <div className="bg-black rounded-3xl overflow-hidden border border-white/10 aspect-video shadow-2xl">
                    ...
                    {selectedModule.videoSource === 'file' && (
                      <div className="relative group">
                        <input type="file" accept="video/*" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
                        <div className="w-full bg-white/5 border-2 border-dashed border-white/10 p-12 rounded-[2rem] text-center"><UploadCloud className="mx-auto mb-4 text-zinc-700 group-hover:text-[#00E5FF]" size={40} /><p className="text-zinc-500 text-[9px] font-black uppercase">Cargar Video de Clase</p></div>
                      </div>
                    )}
                  </div>
                )}
                ...
                {selectedModule.type === 'embed' && (
                  <div className="space-y-6">
                    ...
                    {selectedModule.content && <div className="bg-black rounded-3xl overflow-hidden border border-white/10 aspect-video shadow-2xl"><div className="w-full h-full" dangerouslySetInnerHTML={{ __html: selectedModule.content }} /></div>}
                  </div>
                )}
              </div>
            ) : <div className="h-full flex items-center justify-center text-zinc-700 uppercase tracking-[0.5em] text-[10px] italic">Sincroniza un nodo neural para comenzar</div>}
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 animate-in zoom-in-95 duration-500">
          <div className="space-y-6 bg-[#050505] p-10 rounded-[3rem] border border-white/5 shadow-2xl">
            <h3 className="text-white text-sm font-black italic uppercase tracking-widest leading-none">Diplomado Dinámico</h3>
            <div className="space-y-3">
              {['showName', 'showCourse', 'showDate'].map(v => (
                <div key={v} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10 hover:border-[#00E5FF]/20 transition-all">
                  <span className="text-white text-[10px] font-bold uppercase">{v === 'showName' ? 'Estudiante' : v === 'showCourse' ? 'Módulo' : 'Temporalidad'}</span>
          ...
          <div className="bg-white rounded-[2rem] aspect-[1.414/1] flex flex-col items-center justify-center p-12 text-center text-black shadow-2xl relative overflow-hidden">
             ...
             {certSettings.showName && <h4 className="text-5xl font-black uppercase italic mb-4 relative z-10 tracking-tighter leading-none">Freddy Moncayo</h4>}
